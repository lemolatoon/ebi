...
def calculate_precision_and_dump_to_json(dataset_metas, output_dir):
    precision_data = {}
    precision_data = (
        json.load(open(os.path.join(output_dir, "precision_data.json"), "r"))
        if os.path.exists(os.path.join(output_dir, "precision_data.json"))
        else {}
    )
    ...
    for dataset_name, column_name in tqdm(dataset_metas):
        if f"{dataset_name.replace('/', '_')}_{column_name}" in precision_data:
            print(
                f"Skipping dataset '{dataset_name}' with column '{column_name}' as it has already been processed"
            )
            continue
        try:
            # Load the dataset
            ...
            precisions = [
                calculate_precision(x) for array in column_data for x in array
            ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Process datasets and calculate precision."
    )
    parser.add_argument(
        "output_dir",
        type=str,
        help="Directory to output the .bin files and precision_data.json",
    )
